...
public:
    ...
    {
        readIndex += BufferUtil::read<T>(data + readIndex, value);
    }
    ...
    [[nodiscard]] int getReadIndex() const
    {
        return readIndex;
    }

    [[nodiscard]] int getReadableBytes() const
    {
        return length - readIndex;
    }

    [[nodiscard]] bool ensureReadableBytes(int numBytes) const
    {
        return getReadableBytes() >= numBytes;
    }
