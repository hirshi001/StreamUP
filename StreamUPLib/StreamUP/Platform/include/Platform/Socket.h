...
{
public:
    Socket() { impl = std::make_unique<SocketImpl>(); }

    int bind(const Address &address) { return impl->bind(address); }


    int sendTo(const uint8_t *buffer, int len, const Address &address) { return impl->sendTo(buffer, len); }
